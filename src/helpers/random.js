...
// generates a random combination of each of the array items
export const generateRandomCombination = (array) => {
  // get number of array
  const numberOfArrays = array.length;

  // confirm all arrays are the same length
  const sameLength = array.every((arr) => arr.length === numberOfArrays);

  if (!sameLength) {
    throw new Error('All arrays must be the same length');
  }

  // create a new array to hold the random combinations
  const combinations = new Array(numberOfArrays).fill(new Array(numberOfArrays));
}
